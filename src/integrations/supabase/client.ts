...
export const updateDeliveryNoteItems = async (deliveryNoteId: string, items: any[]) => {
  try {
    ...
    // Filter out items without valid product IDs and map to proper format
    const validItems = items.filter(item => item.productId && item.productId.trim() !== '');
    
    if (validItems.length > 0) {
      const itemLinks = validItems.map(item => ({
        deliverynoteid: deliveryNoteId,
        itemid: item.productId  // Use productId as the itemid
      }));
